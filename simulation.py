...
"""
def runsim(v0:int, iterations:int, n:int, W:np.array, method="outer", proportionalnoise=0, constantnoise=0, training=False):
...
def runplot():
    ...
    for i, pop in enumerate(data2x2):
        plt.plot(pop)
        with open(f"frequency_logs/{i}_.txt", "w") as f:
            f.write(str(pop))
...
def generate_training_data(format, save=False):
    ...
    for i in range(n):
        ...
        data2x2, meanfit2x2, training_data = runsim(init_conditions, 100, 4, W, constantnoise=0, proportionalnoise=0, training=format)
        total_training_data = {**total_training_data, **training_data}
        print(f"{i+1}/{n} complete {len(total_training_data)}", end="\r")
    if save:
        with open(f"training_data/{format}_training_data.json", "w") as f:
